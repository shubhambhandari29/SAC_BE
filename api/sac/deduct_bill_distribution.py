...
from core.models.deduct_bill_distribution import DeductBillDistributionEntry
from services.auth_service import get_current_user_from_token
from services.sac.deduct_bill_distribution_service import (
...
@router.post("/upsert")
async def upsert_distribution(payload: list[DeductBillDistributionEntry]):
    rows = [entry.model_dump() for entry in payload]
    return await upsert_distribution_service(rows)


@router.post("/delete")
async def delete_distribution(payload: list[DeductBillDistributionEntry]):
    rows = [entry.model_dump() for entry in payload]
